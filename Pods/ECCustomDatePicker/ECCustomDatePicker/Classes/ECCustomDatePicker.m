...
// Constants sizes :
static CGFloat const kECCustomDatePickerHeight = 260.0f;

static CGFloat const kECCustomDatePickerHeaderHeight = 44.0f;

static CGFloat const kECCustomDatePickerButtonHeaderWidth = 40.0f;

static CGFloat const kECCustomDatePickerHeaderBottomMargin = 1.0f;

static CGFloat const kECCustomDatePickerScrollViewDaysWidth = 90.0f;

static CGFloat const kECCustomDatePickerScrollViewMonthWidth = 140.0f;

static CGFloat const kECCustomDatePickerScrollViewDateWidth = 165.0f;

static CGFloat const kECCustomDatePickerScrollViewLeftMargin = 1.0f;

static CGFloat const kECCustomDatePickerScrollViewItemHeight = 45.0f;

static CGFloat const kECCustomDatePickerLineWidth = 2.0f;

static CGFloat const kECCustomDatePickerLineMargin = 15.0f;

static CGFloat const kECCustomDatePickerPadding = 15.0f;

// Constants times :
static CGFloat const kECCustomDatePickerAnimationDuration = 0.4f;

// Constants fonts
#define kECCustomDatePickerTitleFont [UIFont systemFontOfSize:18.0]
#define kECCustomDatePickerLabelFont [UIFont systemFontOfSize:18.0]
#define kECCustomDatePickerLabelSelectedFont [UIFont systemFontOfSize:20.0];

// Constants colors :
#define kECCustomDatePickerTintColor [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1.0]
#define kECCustomDatePickerHighlightColor [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1.0]
#define kECCustomDatePickerBackgroundColor [UIColor colorWithRed:244/255.0 green:242/255.0 blue:248/255.0 alpha:1.0]
#define kECCustomDatePickerScrolViewBackgroundColor [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0]
#define kECCustomDatePickerTitleFontColor kECCustomDatePickerTintColor
#define kECCustomDatePickerFontColorLabel kECCustomDatePickerTintColor
#define kECCustomDatePickerFontColorLabelSelected kECCustomDatePickerHighlightColor
#define kECCustomDatePickerBackgroundColorLines kECCustomDatePickerHighlightColor
...
@interface ECCustomDatePicker () <UIScrollViewDelegate>
{
    // Lines :
...
- (instancetype)initWithSuperView:(UIView*)superView {
    ...
    if (self = [super initWithFrame:CGRectMake(0.0, superView.frame.size.height, superView.frame.size.width, kECCustomDatePickerHeight)]) {
        _datePickerMode = ECCustomDatePickerModeDate;
        [superView addSubview:self];
        _superView = superView;
        _minYear = 1900;
        self.tintColor = kECCustomDatePickerTintColor;
        self.highlightColor = kECCustomDatePickerHighlightColor;
...
- (void)setupControl {
    ...
    if (self.datePickerMode == ECCustomDatePickerModeDateAndTime) {
        _dates = [self getDates];
    }
    
    // Background :
    self.backgroundColor = kECCustomDatePickerBackgroundColor;
    
    // Date Selectors :
    if (self.datePickerMode == ECCustomDatePickerModeDate) {
        [self buildSelectorYearsOffsetX:0.0 andWidth:kECCustomDatePickerScrollViewMonthWidth];
        [self buildSelectorMonthsOffsetX:(_scrollViewYears.frame.size.width + kECCustomDatePickerScrollViewLeftMargin) andWidth:kECCustomDatePickerScrollViewDaysWidth];
        [self buildSelectorDaysOffsetX:(_scrollViewMonths.frame.origin.x + _scrollViewMonths.frame.size.width + kECCustomDatePickerScrollViewLeftMargin) andWidth:(self.frame.size.width - (_scrollViewMonths.frame.origin.x + _scrollViewMonths.frame.size.width + kECCustomDatePickerScrollViewLeftMargin))];
    }
    
    // Time Selectors :
    if (self.datePickerMode == ECCustomDatePickerModeTime) {
        [self buildSelectorHoursOffsetX:0.0 andWidth:((self.frame.size.width / 3.0) - kECCustomDatePickerScrollViewLeftMargin)];
        [self buildSelectorMinutesOffsetX:(self.frame.size.width / 3.0) andWidth:((self.frame.size.width / 3.0) - kECCustomDatePickerScrollViewLeftMargin)];
        [self buildSelectorSecondsOffsetX:((self.frame.size.width / 3.0) * 2.0) andWidth:(self.frame.size.width / 3.0)];
    }
    
    // Date & Time Selectors :
    if (self.datePickerMode == ECCustomDatePickerModeDateAndTime) {
        [self buildSelectorDatesOffsetX:0.0 andWidth:kECCustomDatePickerScrollViewDateWidth];
        [self buildSelectorHoursOffsetX:(kECCustomDatePickerScrollViewDateWidth + kECCustomDatePickerScrollViewLeftMargin) andWidth:(((self.frame.size.width - kECCustomDatePickerScrollViewDateWidth) / 2.0) - kECCustomDatePickerScrollViewLeftMargin)];
        [self buildSelectorMinutesOffsetX:(kECCustomDatePickerScrollViewDateWidth + kECCustomDatePickerScrollViewLeftMargin + ((self.frame.size.width - kECCustomDatePickerScrollViewDateWidth) / 2.0)) andWidth:(((self.frame.size.width - kECCustomDatePickerScrollViewDateWidth) / 2.0) - kECCustomDatePickerScrollViewLeftMargin)];
    }
    
    if (self.datePickerMode == ECCustomDatePickerModeYearAndMonth) {
        [self buildSelectorYearsOffsetX:0.0 andWidth:self.frame.size.width * 0.5];
        [self buildSelectorMonthsOffsetX:(_scrollViewYears.frame.size.width + kECCustomDatePickerScrollViewLeftMargin) andWidth:self.frame.size.width * 0.5];
...
- (void)buildSelectorDaysOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    // ScrollView Days :
    _scrollViewDays = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewDays.tag = ScrollViewTagValue_DAYS;
    _scrollViewDays.delegate = self;
    _scrollViewDays.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineDaysTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewDays.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewDays.frame.origin.y + (_scrollViewDays.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineDaysTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineDaysTop];
    
    _lineDaysBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewDays.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewDays.frame.origin.y + (_scrollViewDays.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsDays {
    ...
    CGFloat offsetContentScrollView = (_scrollViewYears.frame.size.height - kECCustomDatePickerScrollViewItemHeight) / 2.0;
    ...
    for (int i = 0; i < _days.count; i++) {
        ...
        UILabel *labelDay = [[UILabel alloc] initWithFrame:CGRectMake(0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewDays.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelDay.text = day;
        labelDay.font = kECCustomDatePickerLabelFont;
    ...
    _scrollViewDays.contentSize = CGSizeMake(_scrollViewDays.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * _days.count) + (offsetContentScrollView * 2));
}
...
- (void)buildSelectorMonthsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _scrollViewMonths = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewMonths.tag = ScrollViewTagValue_MONTHS;
    _scrollViewMonths.delegate = self;
    _scrollViewMonths.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineMonthsTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewMonths.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewMonths.frame.origin.y + (_scrollViewMonths.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineMonthsTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineMonthsTop];
    
    _lineMonthsBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewMonths.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewMonths.frame.origin.y + (_scrollViewMonths.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsMonths {
    ...
    for (int i = 0; i < self.months.count; i++) {
        ...
        UILabel *labelDay = [[UILabel alloc] initWithFrame:CGRectMake(0.0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewMonths.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelDay.text = day;
        labelDay.font = kECCustomDatePickerLabelFont;
        labelDay.textAlignment = NSTextAlignmentCenter;
        labelDay.textColor = self.tintColor;
    ...
    _scrollViewMonths.contentSize = CGSizeMake(_scrollViewMonths.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * self.months.count) + (offsetContentScrollView * 2));
}
...
- (void)buildSelectorYearsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _scrollViewYears = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewYears.tag = ScrollViewTagValue_YEARS;
    _scrollViewYears.delegate = self;
    _scrollViewYears.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineYearsTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewYears.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewYears.frame.origin.y + (_scrollViewYears.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineYearsTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineYearsTop];
    
    _lineYearsBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewYears.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewYears.frame.origin.y + (_scrollViewYears.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsYears {
    ...
    for (int i = 0; i < _years.count; i++) {
        ...
        UILabel *labelDay = [[UILabel alloc] initWithFrame:CGRectMake(0.0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewYears.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelDay.text = day;
        labelDay.font = kECCustomDatePickerLabelFont;
        labelDay.textAlignment = NSTextAlignmentCenter;
        labelDay.textColor = self.tintColor;
    ...
    _scrollViewYears.contentSize = CGSizeMake(_scrollViewYears.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * _years.count) + (offsetContentScrollView * 2));
}
...
- (void)buildSelectorDatesOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    // ScrollView Dates
    _scrollViewDates = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewDates.tag = ScrollViewTagValue_DATES;
    _scrollViewDates.delegate = self;
    _scrollViewDates.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineDatesTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewDates.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewDates.frame.origin.y + (_scrollViewDates.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineDatesTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineDatesTop];
    
    _lineDatesBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewDates.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewDates.frame.origin.y + (_scrollViewDates.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsDates {
    ...
    CGFloat offsetContentScrollView = (_scrollViewDates.frame.size.height - kECCustomDatePickerScrollViewItemHeight) / 2.0;
    ...
    for (int i = 0; i < _dates.count; i++) {
        ...
        UILabel *labelDate = [[UILabel alloc] initWithFrame:CGRectMake(0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewDates.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelDate.text = hour;
        labelDate.font = kECCustomDatePickerLabelFont;
    ...
    _scrollViewDates.contentSize = CGSizeMake(_scrollViewDates.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * _dates.count) + (offsetContentScrollView * 2));
}
...
- (void)buildSelectorHoursOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    // ScrollView Hours :
    _scrollViewHours = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewHours.tag = ScrollViewTagValue_HOURS;
    _scrollViewHours.delegate = self;
    _scrollViewHours.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineHoursTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewHours.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewHours.frame.origin.y + (_scrollViewHours.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineHoursTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineHoursTop];
    
    _lineHoursBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewHours.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewHours.frame.origin.y + (_scrollViewHours.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsHours {
    ...
    CGFloat offsetContentScrollView = (_scrollViewHours.frame.size.height - kECCustomDatePickerScrollViewItemHeight) / 2.0;
    ...
    for (int i = 0; i < self.hours.count; i++) {
        ...
        UILabel *labelHour = [[UILabel alloc] initWithFrame:CGRectMake(0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewHours.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelHour.text = hour;
        labelHour.font = kECCustomDatePickerLabelFont;
    ...
    _scrollViewHours.contentSize = CGSizeMake(_scrollViewHours.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * self.hours.count) + (offsetContentScrollView * 2));
}
...
- (void)buildSelectorMinutesOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    // ScrollView Minutes :
    _scrollViewMinutes = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewMinutes.tag = ScrollViewTagValue_MINUTES;
    _scrollViewMinutes.delegate = self;
    _scrollViewMinutes.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineMinutesTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewMinutes.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewMinutes.frame.origin.y + (_scrollViewMinutes.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineMinutesTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineMinutesTop];
    
    _lineMinutesBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewMinutes.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewMinutes.frame.origin.y + (_scrollViewMinutes.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsMinutes {
    ...
    CGFloat offsetContentScrollView = (_scrollViewMinutes.frame.size.height - kECCustomDatePickerScrollViewItemHeight) / 2.0;
    ...
    for (int i = 0; i < self.minutes.count; i++) {
        ...
        UILabel *labelMinute = [[UILabel alloc] initWithFrame:CGRectMake(0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewMinutes.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelMinute.text = minute;
        labelMinute.font = kECCustomDatePickerLabelFont;
    ...
    _scrollViewMinutes.contentSize = CGSizeMake(_scrollViewMinutes.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * self.minutes.count) + (offsetContentScrollView * 2));
}
...
- (void)buildSelectorSecondsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    // ScrollView Seconds :
    _scrollViewSeconds = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kECCustomDatePickerHeaderHeight + kECCustomDatePickerHeaderBottomMargin, width, self.frame.size.height - kECCustomDatePickerHeaderHeight - kECCustomDatePickerHeaderBottomMargin)];
    _scrollViewSeconds.tag = ScrollViewTagValue_SECONDS;
    _scrollViewSeconds.delegate = self;
    _scrollViewSeconds.backgroundColor = kECCustomDatePickerScrolViewBackgroundColor;
    ...
    _lineSecondsTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewSeconds.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewSeconds.frame.origin.y + (_scrollViewSeconds.frame.size.height / 2) - (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
    _lineSecondsTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineSecondsTop];
    
    _lineSecondsBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewSeconds.frame.origin.x + kECCustomDatePickerLineMargin, _scrollViewSeconds.frame.origin.y + (_scrollViewSeconds.frame.size.height / 2) + (kECCustomDatePickerScrollViewItemHeight / 2), width - (2 * kECCustomDatePickerLineMargin), kECCustomDatePickerLineWidth)];
...
- (void)buildSelectorLabelsSeconds {
    ...
    CGFloat offsetContentScrollView = (_scrollViewSeconds.frame.size.height - kECCustomDatePickerScrollViewItemHeight) / 2.0;
    ...
    for (int i = 0; i < self.seconds.count; i++) {
        ...
        UILabel *labelSecond = [[UILabel alloc] initWithFrame:CGRectMake(0, (i * kECCustomDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewSeconds.frame.size.width, kECCustomDatePickerScrollViewItemHeight)];
        labelSecond.text = second;
        labelSecond.font = kECCustomDatePickerLabelFont;
    ...
    _scrollViewSeconds.contentSize = CGSizeMake(_scrollViewSeconds.frame.size.width, (kECCustomDatePickerScrollViewItemHeight * self.seconds.count) + (offsetContentScrollView * 2));
}
...
-(void)show {
    ...
    if (self.datePickerMode == ECCustomDatePickerModeDate || self.datePickerMode == ECCustomDatePickerModeDateAndTime) {
    ...
    if (self.datePickerMode == ECCustomDatePickerModeTime || self.datePickerMode == ECCustomDatePickerModeDateAndTime) {
    ...
    [UIView animateWithDuration:kECCustomDatePickerAnimationDuration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.frame = CGRectMake(self.frame.origin.x, _superView.frame.size.height - kECCustomDatePickerHeight, self.frame.size.width, self.frame.size.height);
...
- (void)setDatePickerMode:(ECCustomDatePickerMode)mode {
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (self.datePickerMode != ECCustomDatePickerModeDateAndTime && self.datePickerMode != ECCustomDatePickerModeTime) {
...
- (int)getIndexForScrollViewPosition:(UIScrollView *)scrollView {
    ...
    CGFloat offsetContentScrollView = (scrollView.frame.size.height - kECCustomDatePickerScrollViewItemHeight) / 2.0;
    CGFloat offetY = scrollView.contentOffset.y;
    CGFloat index = floorf((offetY + offsetContentScrollView) / kECCustomDatePickerScrollViewItemHeight);
    index = (index - 1);
    return index;
}
...
- (void)setScrollView:(UIScrollView*)scrollView atIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    if (animated) {
        ...
        [UIView setAnimationDuration:kECCustomDatePickerAnimationDuration];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
    }
    
    scrollView.contentOffset = CGPointMake(0.0, (index * kECCustomDatePickerScrollViewItemHeight));
...
- (void)highlightLabelInArray:(NSMutableArray*)labels atIndex:(NSInteger)index {
    ...
    for (int i = 0; i < labels.count; i++) {
        ...
        if (i != index) {
            ...
            label.font = kECCustomDatePickerLabelFont;
        } else {
            label.textColor = self.highlightColor;
            label.font = kECCustomDatePickerLabelSelectedFont;
        }
    }
}
...
- (UIView *)headerView {
    if (!_headerView) {
        _headerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, kECCustomDatePickerHeaderHeight)];
        // Button Cancel
        UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(kECCustomDatePickerPadding, 0.0, kECCustomDatePickerButtonHeaderWidth, kECCustomDatePickerHeaderHeight)];
        ...
        // Button confirm
        UIButton *sureButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - kECCustomDatePickerButtonHeaderWidth - kECCustomDatePickerPadding, 0.0, kECCustomDatePickerButtonHeaderWidth, kECCustomDatePickerHeaderHeight)];
        ...
        // Label Title
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(cancelButton.frame) + kECCustomDatePickerPadding, 0.0, self.frame.size.width - ((kECCustomDatePickerButtonHeaderWidth + kECCustomDatePickerPadding * 2) * 2 ), kECCustomDatePickerHeaderHeight)];
        _titleLabel.text = self.title;
        _titleLabel.font = kECCustomDatePickerTitleFont;
